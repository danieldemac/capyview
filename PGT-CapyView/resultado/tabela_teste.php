...
class UserDependent extends Model
{
    ...
    {
        try {
            $tabela_testes = static::select(DB::raw("name, relationship, birth"))
            ...
            return [
                'count_data'=>$tabela_testes->count(),
                'data'      =>$tabela_testes->offset($start)->limit($limit)->orderBy($order, $dir)->get(), 
